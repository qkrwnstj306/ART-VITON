...
class VITONHDDataset(Dataset):
    def __init__(
        ...
        im_names = []
        c_names = []
        with open(opj(self.drd, f"test_pairs.txt"), "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        if self.transform_size is None and self.transform_color is None:
            ...
            wrp_cloth = imread_for_albu(opj(self.drd, self.data_type, wrp_name, self.c_names[self.pair_key][idx])) if not self.is_test else np.zeros_like(agn_mask)
            wrp_cloth_mask = imread_for_albu(opj(self.drd, self.data_type, wrp_mask_name, self.c_names[self.pair_key][idx]), is_mask=True) if not self.is_test else np.zeros_like(agn_mask)
