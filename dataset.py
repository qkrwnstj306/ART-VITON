...
class VITONHDDataset(Dataset):
    def __init__(
        ...
        with open(opj(self.drd, f"figure_pairs.txt"), "r") as f:
            for line in f.readlines():
                im_name, c_name = line.strip().split()
                #if im_name == "08800_00.jpg": # 00126_00
...
class SHHQDataset(Dataset):
    def __init__(
        ...
        c_names = []
        #with open(opj(self.drd, f"{self.data_type}_pairs.txt"), "r") as f:
        with open(opj(self.drd, f"final_figure_pairs.txt"), "r") as f:
            for line in f.readlines():
                im_name, c_name = line.strip().split()
                #if im_name == "08800_00.jpg": # 00126_00
                im_names.append(im_name)
    ...
    def __getitem__(self, idx):
        ...
        if self.transform_size is None and self.transform_color is None:
            ...
            agn = imread_for_albu(opj(self.drd, self.data_type, "agnostic-v3.2", self.im_names[idx]), use_resize=True)
            agn_mask = imread_for_albu(opj(self.drd, self.data_type, "agnostic-mask", self.im_names[idx].replace(".jpg", "_mask.png")), is_mask=True, use_resize=True )
            cloth = imread_for_albu(opj(self.drd, self.data_type, "cloth", self.c_names[self.pair_key][idx]), use_resize=True)
            cloth_mask = imread_for_albu(opj(self.drd, self.data_type, "cloth-mask", self.c_names[self.pair_key][idx]), is_mask=True, cloth_mask_check=True, use_resize=True)
            
            gt_cloth_warped_mask = imread_for_albu(
                opj(self.drd, self.data_type, "gt_cloth_warped_mask", self.im_names[idx]),
                is_mask=True, use_resize=True
            ) if not self.is_test else np.zeros_like(agn_mask)
                
            image = imread_for_albu(opj(self.drd, self.data_type, "image", self.im_names[idx]), use_resize=True)
            image_densepose = imread_for_albu(opj(self.drd, self.data_type, "image-densepose", self.im_names[idx]), use_resize=True)
